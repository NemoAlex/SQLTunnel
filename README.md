...
成功响应：`docs/openapi.json` 的 JSON 内容。
...
- `apiKey`：必填。client 的 API key，必须放在请求 body 中。
...
- `connections[].id`：db server id，查询时作为 `connectionId` 使用。
- `connections[].type`：数据库类型，`mysql` 或 `postgres`。
- `connections[].permission`：当前 client 对该 db server 的权限，`read` 或 `write`。
- `connections[].maxRows`：该 client 在该 db server 上的最终最大返回行数。
- `connections[].timeoutMs`：该 client 在该 db server 上的最终查询超时时间。
- `connections[].ssh`：是否通过 SSH tunnel 连接。
...
- `apiKey`：必填。client 的 API key，必须放在请求 body 中。
- `connectionId`：必填。目标 db server id。
- `sql`：必填。要执行的 SQL。
...
- `connectionId`：本次查询使用的 db server id。
