...
`connectionId` 可以固定为某个业务库，也可以让 Agent 先调用 `/connections` 查看可用连接后再选择。

### 3. Agent 提示词

下面这份可以直接放到 Agent 的 Instructions / System Prompt 中。把 `prod-postgres` 换成你的默认 `connectionId`；如果希望 Agent 自己选择连接，保留“需要时先调用 /connections”这一条。
...
- connectionId列表如下
  - XX: XX业务的数据库
- 如果不确定可用连接，先调用 /connections 查看可访问的数据库连接。
...
错误处理规则：
- 如果工具返回 UNAUTHENTICATED、FORBIDDEN、CONNECTION_NOT_FOUND 或 INVALID_REQUEST，简要说明查询失败原因，并提示用户检查配置或权限。
...
- 用用户的语言回答。
- 优先给结论，再补充查询依据。
- 当答案依赖某次查询时，简短说明使用了哪个 connectionId 和大致查询条件。
```
### 4. 知识库
