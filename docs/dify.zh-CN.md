...
`dbServerId` 可以固定为某个业务库，也可以让 Agent 先调用 `/schema` 的 `list_databases` 操作查看可用数据库后再选择。

### 3. Agent 提示词

下面这份可以直接放到 Agent 的 Instructions / System Prompt 中。把 `prod-postgres` 换成你的默认 `dbServerId`；如果希望 Agent 自己选择 db server，保留“需要时先调用 /schema 的 list_databases 操作”这一条。
...
- dbServerId 列表如下
  - XX: XX业务的数据库
- 如果不确定可用 db server，先调用 /schema，operation 使用 list_databases。
